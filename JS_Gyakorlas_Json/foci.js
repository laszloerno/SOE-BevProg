...
function reset() {
    document.getElementById("talatok").innerHTML = ''
}


function searchPlayer() {
    let nev = document.getElementById("playerName").value
    let talalatok = []
    console.log(nev)
    ...
    for (let csapat of csapatok) {
        for (let player of csapat.players) {
            if (player.name.toLowerCase().includes(nev.toLowerCase())) {
                talalatok.push(
                    {
                        csapat_neve: csapat.teamName,
                        jatekos_neve: player.name
                    }
                )
    ...
    for (let talalat of talalatok) {
        document.getElementById("talatok").innerHTML +=
            `<li>${talalat.jatekos_neve} (${talalat.csapat_neve})</li>`
    }

}
function searchByGoal() {
    reset()
    let golokSzama = document.getElementById("goal").value
    let talalatok = []
    if (golokSzama == 0) {
        reset()
        return
...
function topPlayer() {
    ...
    tmpGollovok.sort((a,b) => b.rugott_gol - a.rugott_gol)
    
    for (let talalat of tmpGollovok) {

        document.getElementById("topJatekosok").innerHTML +=
            `<li>${talalat.jatekos_neve} (${talalat.csapat_neve}) rúgott gólok: ${talalat.rugott_gol}</li>`
    }
